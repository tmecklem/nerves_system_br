...
define nerves_pkg_info_start
	echo "name,$(1)" > $(BUILD_DIR)/nerves-pkg-$(1).txt
	echo "version,$($(PKG)_VERSION)" >> $(BUILD_DIR)/nerves-pkg-$(1).txt
	echo "url,$($(PKG)_SITE)" >> $(BUILD_DIR)/nerves-pkg-$(1).txt
	echo "license,$($(PKG)_LICENSE)" >> $(BUILD_DIR)/nerves-pkg-$(1).txt
	echo "dependencies,$(foreach dep,$($(PKG)_DEPENDENCIES),$(dep)-$($(call UPPERCASE,$(dep))_VERSION))" >> $(BUILD_DIR)/nerves-pkg-$(1).txt
endef

define nerves_pkg_info_end
        comm -13 $($(PKG)_DIR)/.br_filelist_before $($(PKG)_DIR)/.br_filelist_after | \
                while read hash file ; do \
                        echo "installed,$${file}" >> $(BUILD_DIR)/nerves-pkg-$(1).txt ; \
                done
endef
